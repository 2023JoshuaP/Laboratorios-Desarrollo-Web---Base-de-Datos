...
app.post('/insertar', verifyUser, (req, res) => {
    const { id, nombre, apell_pat, apell_mat, edad  } = req.body;
    const sql = "INSERT INTO persona (id, nombre, apell_pat, apell_mat, edad) VALUES (?, ?, ?, ?, ?)";
    db.query(sql, [id, nombre, apell_pat, apell_mat, edad], (err, result) => {
        if (err) {
            console.error('Error en la consulta SQL:', err);
            return res.status(500).json({ message: 'Error en el servidor' });
        }
...
app.get('/mostrarIDimpares', verifyUser, (req, res) => {
    const sql = "CALL MostrarIDImpar()";
    db.query(sql, (err, data) => {
        if (err) {
            console.error('Error en la consulta SQL:', err);
            return res.status(500).json({ message: 'Error en el servidor' });
